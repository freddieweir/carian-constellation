...
3. ✅ **Configured CloudWatch billing alarm**
   ...
   - Email notification to: aws@freddieweir.com
...
if [ "$CLUSTER" != "None" ] && [ -n "$CLUSTER" ]; then
  ...
  if [ $AGE_HOURS -gt 8 ]; then
    ...
    echo "WARNING: EKS cluster $CLUSTER has been running for $AGE_HOURS hours" | \
      mail -s "Constellation Alert: Long-running infrastructure" aws@freddieweir.com
  fi
fi
```
...
- **SNS Topic**: arn:aws:sns:us-east-1:659093129022:constellation-billing-alerts
